...
async def file_upload_handler(client: Client, message: Message):
    ...
    try:
        ...
        # File naam ke saath RAJ DEV credit caption lagao
        original_name = info["file_name"]
        if "." in original_name:
            name_part = original_name.rsplit(".", 1)[0]
        else:
            name_part = original_name
        branded_caption = name_part + " | RAJ DEV" + chr(10) + chr(10) + "🔖 @raj_dev_01"
        ...
        # 5. Channel post
        # ⚠️ IMPORTANT: Channel mein hamesha DIRECT BOT LINK jaata hai
        # Shortener bot ke andar apply hota hai — user ke tier ke hisab se
        # Premium → direct file | Regular → shortener → file
        channel_link = direct_link
