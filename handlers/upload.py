...
async def _get_thumb_bytes(client: Client, message: Message) -> bytes | None:
    ...
    Agar thumbnail nahi hai ya download fail ho to None.
    """
    try:
        # Video thumbs list se pehla thumb lo
        if message.video and message.video.thumbs:
            thumb = message.video.thumbs[0]
            buf = io.BytesIO()
            await client.download_media(thumb.file_id, file_name=buf)
            buf.seek(0)
            data = buf.read()
            return data if data else None
        # Animation/GIF thumb
        if message.animation and message.animation.thumbs:
            thumb = message.animation.thumbs[0]
            buf = io.BytesIO()
            await client.download_media(thumb.file_id, file_name=buf)
            buf.seek(0)
            data = buf.read()
            return data if data else None
        # Document mein embedded video ke thumb
