...
async def handle_file_request(client: Client, message: Message, file_id: str):
    ...
    try:
        ...
        # Admin / Premium  → hamesha direct URL (shortener kabhi nahi)
        # Regular + short ON + short_url stored → short URL
        # Regular + short OFF ya short_url nahi → direct URL
        # ──────────────────────────────────────────────────────
        if is_admin or is_premium:
            user_link   = direct_url
            link_label  = "🔗 Direct Link" + (" 👑 Premium" if is_premium else " 🔧 Admin")
            bypass_note = "_(Shortener bypass — Premium access)_" if is_premium else ""
