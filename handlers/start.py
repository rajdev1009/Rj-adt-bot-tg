...
async def handle_file_request(client: Client, message: Message, file_id: str):
    ...
    try:
        ...
        if is_admin or is_premium:
            ...
            # short_url_db check karo — agar hai to seedha use karo
            # Agar nahi hai (purani file) to real-time shorten karo
            # API key DIRECTLY Config (env var) se lega — MongoDB se NAHI
            if short_url_db:
                user_link = short_url_db
            else:
                api_key = Config.SHORTENER_API  # ✅ Env var se directly
                if api_key and direct_url:
                    shortened = await shorten_url(direct_url, api_key, Config.SHORTENER_SITE)
                    # Agar shortener kaam kiya to use karo, warna direct
                    user_link = shortened if shortened != direct_url else direct_url
                    # Future ke liye DB mein bhi save kar do
                    if user_link != direct_url:
                        await DB.save_file(
                            file_id    = file_id,
                            file_name  = file_name,
                            message_id = file_doc["message_id"],
