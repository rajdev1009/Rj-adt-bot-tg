...
║     Step 1: /start file_xxx aata hai                         ║
║             → Already verified today? → Seedha file do       ║
║             → Not verified? → Token banao, shortlink bhejo   ║
║     Step 2: User shortlink visit kare                        ║
║             → /start verify_{user_id}_{token}_{file_id}      ║
║             → Token valid? → Verify karo → File do           ║
║             → Token invalid? → Error dikhao                  ║
║                                                              ║
...
#  Payload format: verify_{user_id}_{token}_{file_id}
# ══════════════════════════════════════════════════════════════

async def handle_token_verify(client: Client, message: Message, payload: str):
    """
    User shortlink visit karke wapas aaya.
    Token verify karo, phir file do.
    """
    try:
        # "verify_" ke baad: {user_id}_{token}_{file_id}
        parts     = payload[7:].split("_", 2)  # max 3 parts
        req_uid   = int(parts[0])
        token     = parts[1]
        file_id   = parts[2]
    except Exception:
    ...
    if not is_valid:
        await message.reply(
            ...
            ]) if Config.get_channel_url() else None
        )
        return

    # ✅ Token valid — verify karo aur file do
    ...
    await handle_file_request(client, message, file_id, skip_verification=True)
...
async def handle_file_request(client: Client, message: Message, file_id: str,
    ...
    """
    skip_verification=True tab pass karo jab token already verify ho chuka ho.
    """
    user = message.from_user
    ...
    try:
        ...
        # Already verified aaj ke liye?
        if skip_verification or await check_verification(user.id):
            await search_msg.delete()
            await _deliver_file(client, message, file_doc, file_id, False, False)
            return

        # ✅ Token banao + shortlink bhejo
        # Verify link format: https://t.me/BotName?start=verify_{user_id}_{token}_{file_id}
        ...
        verify_deep_link = (
            ...
            f"?start=verify_{user.id}_TOKEN_{file_id}"
        )
        # get_token function token generate karega aur verify_deep_link mein replace karega
        # Hum token ko link mein manually embed karenge
        import random, string as _string
        token = ''.join(random.choices(_string.ascii_letters + _string.digits, k=7))
