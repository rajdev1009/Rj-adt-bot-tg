...
#  VERIFIED = { user_id: "YYYY-MM-DD" }  (valid for today only)
# ══════════════════════════════════════════════════════════════
...
async def get_token(user_id: int, base_link: str) -> str:
    """
    User ke liye ek unique token generate karo.
    Token ko shortlink mein embed karo aur shorten karke return karo.
    base_link = bot ka deep link, e.g. https://t.me/BotName?start=file_xxx
    Format of verify link: https://t.me/BotName?start=verify-{user_id}-{token}-{file_id_part}
    
    Yahan hum base_link ko hi shorten karte hain with token appended as fragment.
    """
    token = _generate_token(7)
    TOKENS[user_id] = {token: False}  # False = not yet used

    # Verify link = base_link + token info
    # Bot is link ko receive karega as: verify_{user_id}_{token}_{file_payload}
    verify_link = base_link  # shortener ke baad yeh verify link banega

    api_key = Config.SHORTENER_API
    site    = Config.SHORTENER_SITE

    shortened = await shorten_url(verify_link, api_key, site)
    return shortened, token


async def check_token(user_id: int, token: str) -> bool:
    """
    Token valid hai aur use nahi hua — True return karo.
    """
    if user_id in TOKENS:
        tkn_map = TOKENS[user_id]
        if token in tkn_map:
            is_used = tkn_map[token]
            return not is_used  # False = not used = valid
    return False


async def verify_user(user_id: int, token: str):
    """
    Token ko used mark karo aur user ko aaj ke liye verified karo.
    """
    TOKENS[user_id] = {token: True}
    tz    = pytz.timezone('Asia/Kolkata')
    today = date.today()
    VERIFIED[user_id] = str(today)
    logger.info(f"User {user_id} verified with token {token} on {today}")


async def check_verification(user_id: int) -> bool:
    """
    User aaj ke liye verified hai? True/False return karo.
    Verification sirf aaj ke din ke liye valid hai.
    """
    tz    = pytz.timezone('Asia/Kolkata')
    today = date.today()

    if user_id in VERIFIED:
        exp = VERIFIED[user_id]
        try:
            year, month, day = exp.split('-')
            comp = date(int(year), int(month), int(day))
            return comp >= today
        except Exception:
            return False
    return False
...
async def check_force_subscribe(client, user_id: int) -> bool:
    """
    Check karo ki user UPDATE_CHANNEL ka member hai ya nahi.
    Agar UPDATE_CHANNEL set nahi hai to hamesha True return karo.
    """
