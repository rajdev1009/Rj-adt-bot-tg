...
class MongoDB:
    ...
    @classmethod
    async def reset_all(cls) -> dict:
        """
        Poora database fresh state mein reset karo.
        Sab collections clear honge: users, files, premium, settings.
        Returns: har collection mein kitne documents delete hue.
        """
        r_files    = await cls._files.delete_many({})
        r_users    = await cls._users.delete_many({})
        r_premium  = await cls._premium.delete_many({})
        r_settings = await cls._settings.delete_many({})
        return {
            "files":    r_files.deleted_count,
            "users":    r_users.deleted_count,
            "premium":  r_premium.deleted_count,
