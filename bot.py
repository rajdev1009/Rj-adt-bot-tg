...
async def resolve_channels():
    ...
    if Config.UPDATE_CHANNEL:
        try:
            ...
            Config.UPDATE_CHANNEL = chat.id  # int ID store karo
            # Username bhi save karo buttons ke liye
            if chat.username and not Config.UPDATE_CHANNEL_USERNAME:
                Config.UPDATE_CHANNEL_USERNAME = chat.username
                logger.info(f"   Username: @{chat.username}")
        except (PeerIdInvalid, ChannelInvalid, ChannelPrivate) as e:
            errors.append(
                f"❌ UPDATE_CHANNEL '{Config.UPDATE_CHANNEL}' resolve nahi hua: {e}\n"
                f"   → Bot ko us channel ka Admin banao aur Post Messages permission do."
            )
        except Exception as e:
            errors.append(f"⚠️  UPDATE_CHANNEL resolve warning: {e}")
