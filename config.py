...
class Config:
    ...
    def get_channel_url(cls) -> str:
        """
        Update channel ka t.me URL return karo (inline buttons ke liye).
        Priority:
          1. UPDATE_CHANNEL_USERNAME (runtime mein bot.py ne set kiya)
          2. UPDATE_CHANNEL agar @username format mein hai
          3. Numeric ID → URL nahi banta, empty string return
        """
        # 1. Runtime mein set hua username (bot.py > resolve_channels)
        u = str(cls.UPDATE_CHANNEL_USERNAME).strip().lstrip("@") if cls.UPDATE_CHANNEL_USERNAME else ""
        if u:
            return f"https://t.me/{u}"
        # 2. UPDATE_CHANNEL string username format mein hai
